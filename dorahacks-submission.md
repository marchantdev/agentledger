...
### Key Features
- **Agent Workbench** — Run demo policy agent scenarios (vendor payment, DeFi swap, risk alert) and watch decisions get recorded on-chain in real-time
- **Tamper Detection** — Edit any decision field, re-verify against chain, see instant TAMPERED/VERIFIED result
...
- **Casper Proof Drawer** — Raw on-chain transaction data (named args, block height, contract package) sourced directly from Casper RPC
- **Agent Policy Trace** — Visible rule evaluation steps showing how the agent reached its decision
- **Audit Export** — Download receipt reports in Markdown or JSON

### Tech Stack
- **Smart Contract:** Odra 2.8.0 (Rust) — DecisionRegistry with record_decision, get_decision, get_total_decisions
- **Frontend:** React + Vite + TypeScript + Tailwind CSS, deployed on Vercel
- **Verification:** Client-side SHA-256 hashing + direct Casper RPC calls (no backend trust required)
- **On-chain:** Casper testnet, 6 seed receipts across 4 agent types (blocks 8256786–8256790)
